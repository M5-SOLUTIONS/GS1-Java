...
public class RecursoController {
    ...
    @Operation(summary = "Cadastra um recurso", responses = {
            ...
            @ApiResponse(responseCode = "400", description = "Erro ao cadastrar recurso"),
    ...
    @Operation(summary = "Lista todos os recursos", responses = {
            @ApiResponse(responseCode = "200", description = "Recursos encontrados",
                    content = @Content(array = @ArraySchema(
                            schema = @Schema(implementation = RecursoListagemDTO.class))))
    })
    @GetMapping
    public ResponseEntity<CollectionModel<EntityModel<RecursoListagemDTO>>> listarTodos() {

        List<EntityModel<RecursoListagemDTO>> lista = recursoService
                .readAllRecursos()
                .stream()
    ...
    @GetMapping("/{id}")
    public ResponseEntity<EntityModel<RecursoListagemDTO>> buscarPorId(
            @PathVariable Long id) {

        return ResponseEntity.ok(
                assembler.toModel(recursoService.readRecursoById(id))
        );
    }

    @Operation(summary = "Lista recursos por status", responses = {
            @ApiResponse(responseCode = "200", description = "Recursos encontrados",
                    content = @Content(array = @ArraySchema(
                            schema = @Schema(implementation = RecursoListagemDTO.class)))),
    ...
    public ResponseEntity<CollectionModel<EntityModel<RecursoListagemDTO>>> listarPorStatus(
        ...
        CollectionModel<EntityModel<RecursoListagemDTO>> collection =
                CollectionModel.of(
                        lista,
                        linkTo(methodOn(RecursoController.class)
                                .listarPorStatus(status)).withSelfRel(),
                        linkTo(methodOn(RecursoController.class)
    ...
    @Operation(summary = "Atualiza um recurso", responses = {
            @ApiResponse(responseCode = "200", description = "Recurso atualizado com sucesso",
                    content = @Content(schema = @Schema(implementation = RecursoListagemDTO.class))),
            @ApiResponse(responseCode = "400", description = "Erro ao atualizar recurso"),
            @ApiResponse(responseCode = "404", description = "Recurso ou base não encontrados")
    })
    @PutMapping("/{id}")
